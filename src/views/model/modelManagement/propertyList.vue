<template>
  <div style="height: 100%">
    <div v-if="isShowTable" class="container" v-loading="tableLoading">
      ...
      <div class="properties-table">
        <el-table 
          ...
          <el-table-column prop="front_type" label="属性类型" align="center">
            <template slot-scope="scope">{{ scope.row.front_type | filterPropertyType }}</template>
          </el-table-column>
          <el-table-column prop="required" label="是否必填" align="center">
            <template slot-scope="scope">
              <span v-show="scope.row.required" class="required-icon">
    ...
    <!-- 属性编辑 -->
    <div class="edit-wrap theme-edit-wrap" v-else>
      <div class="content">
        <div class="left" id="leftWrap">
          <div class="left-action">
            <el-button size="small" @click="handleClickCancelAllEdit">取 消</el-button>
            <el-button type="primary" size="small" @click="handleClickSaveAllEdit" :loading="saveEditLoading">保 存</el-button>
          </div>
                    ...
                    <div class="item-row">
                      ...
                      <el-form-item 
                        ...
                        <el-input 
                          ...
                          size="small">
                          <template v-if="(item.front_type === 'int' || item.front_type === 'float') && item.unit" slot="append">{{ item.unit }}</template>
                        </el-input>
                        <!-- 多行文本 -->
                        <el-input
                      ...
                      <!-- 属性分组 -->
                      <div v-show="item.front_type === 'group'" class="group theme-group">
    ...
    <el-dialog
      ...
      <el-form
        ...
        <el-form-item 
          ...
          <!-- 小数 -->
          <el-input-number v-else-if="currentPropertyType === 'float'" :precision="singlePropertyForm.precision" v-model="singlePropertyForm.value"></el-input-number>
          ...
          <el-row
            ...
            <el-col :span="8">
              <el-form-item 
                ...
                label-width="1px"
                :rules="[{required: true, message: '选项名称不能空'},{validator: validateOptionName, trigger: 'blur'}]">
        ...
        <el-form-item
          prop="relationName"
          label="从属标签："
          v-show="currentPropertyType === 'belongReference'">
          <el-input v-model="singlePropertyForm.relationName"></el-input>
        </el-form-item>
        <el-form-item
          prop="relationName"
          label="连接标签："
          v-show="currentPropertyType === 'linkReference'">
          <el-input v-model="singlePropertyForm.relationName"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  ...
  import { filterPropertyType } from '@/utils/filter.js'
  import { modelAction, getAllModels } from '@/api/model.js'
  import { mapMutations, mapGetters } from 'vuex'
  import { constTypeList } from '@/utils/staticData.js'
  ...
  export default {
    ...
    data () {
      ...
      return {
        ...
        constTypeList: constTypeList,
        // propertyList 和 typeList的数据结构必须一致，才能拖拽添加成功
        typeList: constTypeList,
        propertyListForm: {},
        ...
        singlePropertyFormRules: {
          ...
          attribute_code: [
           ...
           {required: true, message: '属性编码不能为空'}
          ],
          refCode: [{required: true, message: '请选择关联模型', trigger: 'change'}]
        },
    ...
    computed: {
      ...
      dragOptions2 () {
        return  {
          ...
          sort: false,
          ghostClass: this.skin === 'light-theme' ? 'light-ghost': 'dark-ghost'
        };
      },
      ...mapGetters([
        'treeList',
    ...
    methods:{
      ...
      getPropertyList () {
        ...
        modelAction(obj).then(res => {
          ...
          if (res.status === 200) {
            ...
            if (list.length === 0 || !list[0].matrix_code) {
      ...
      onStart (ev) {
        ...
        this.currentPropertyType = this.currentDragTypeObj.front_type
        this.currentPropertyTypeName = this.currentDragTypeObj.attribute_name
      },
      // 清空表单
      resetEditForm () {
      ...
      handleClickPropertyItem (item, index, operateFlag) {
        ...
        if (item.front_type === 'belongReference' || item.front_type === 'linkReference') {
          this.$set(this.singlePropertyForm, 'refCode', item.value)
        }
      ...
      handleOptionList () {
        ...
        this.propertyList.forEach(item => {
          ...
          if (item.front_type === 'int') {
            item.value = String(item.value)
          }
          if (item.front_type === 'float') { // 精度变换
            let value = Number(item.value).toFixed(item.precision)
            item.value = String(value)
          }
          if (item.front_type === 'linkReference' || item.front_type === 'belongReference') {
            item.value = item.refCode || item.value
          }
        })
      },
      // 处理确定编辑
      ...
      getParentId () {
        let code = ''
        this.propertyList.forEach(item => {
          if (item.front_type === 'belongReference') {
            code = item.value
          }
        })
        return this.getModuleIdByCode(code)
      },
      getModuleIdByCode (code) {
        let id = ''
